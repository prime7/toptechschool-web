...
const createStyles = (style: ResumeStyle) => StyleSheet.create({
  page: {
    padding: 24,
  ...
  sectionHeader: {
    ...
    borderBottom: style.showSectionHorizontalRule ? `1pt solid ${style.accentColor}` : undefined,
...
const BulletPointsList = ({ bulletPoints, styles }: { bulletPoints: string[], styles: any }) => (
...
const SummarySection = ({ summary, styles }: { summary: string; styles: any }) => (
...
const ExperienceItem = ({ item, styles }: { item: ExperienceItem; styles: any }) => (
...
const ExperienceSection = ({ experience, styles }: { experience: ExperienceItem[]; styles: any }) => {
  ...
  return (
    <>
      {chunks.map((chunk, chunkIndex) => (
        <View key={`chunk-${chunkIndex}`} style={styles.section} break={chunkIndex > 0}>
          {chunkIndex === 0 && <Text style={styles.sectionHeader}>Experience</Text>}
...
const EducationItem = ({ item, styles }: { item: EducationItem; styles: any }) => (
  <View style={{ marginBottom: 10 }} wrap={false}>
    <View style={styles.row}>
      <View>
...
const EducationSection = ({ education, styles }: { education: EducationItem[]; styles: any }) => {
  ...
  return (
    <>
      {chunks.map((chunk, chunkIndex) => (
        <View key={`edu-chunk-${chunkIndex}`} style={styles.section} break={chunkIndex > 0}>
          {chunkIndex === 0 && <Text style={styles.sectionHeader}>Education</Text>}
          {chunk.map((item, index) => (
...
const ProjectItem = ({ project, styles }: { project: ProjectItem; styles: any }) => (
  <View style={{ marginBottom: 10 }} wrap={false}>
    <View style={styles.row}>
      <View>
...
const ProjectsSection = ({ projects, styles }: { projects: ProjectItem[]; styles: any }) => {
  ...
  return (
    <>
      {chunks.map((chunk, chunkIndex) => (
        <View key={`proj-chunk-${chunkIndex}`} style={styles.section} break={chunkIndex > 0}>
          {chunkIndex === 0 && <Text style={styles.sectionHeader}>Projects</Text>}
...
const CertificationItem = ({ certification, styles }: { certification: CertificationItem; styles: any }) => (
  <View style={{ marginBottom: 10 }} wrap={false}>
    <View style={styles.row}>
      <View>
        <Text style={styles.itemHeader}>{certification.name}</Text>
      </View>
      {certification.url && <Text style={styles.date}>{certification.url}</Text>}
    </View>
  </View>
);

const CertificationsSection = ({ certifications, styles }: { certifications: CertificationItem[]; styles: any }) => {
  const chunks: CertificationItem[][] = [];
  let currentChunk: CertificationItem[] = [];
  let currentChunkLength = 0;
  
  const estimateItemSize = () => 0.3;
  ...
  return (
    <>
      {chunks.map((chunk, chunkIndex) => (
        <View key={`cert-chunk-${chunkIndex}`} style={styles.section} break={chunkIndex > 0}>
          {chunkIndex === 0 && <Text style={styles.sectionHeader}>Certifications</Text>}
          {chunk.map((certification, index) => (
            <CertificationItem key={index} certification={certification} styles={styles} />
          ))}
        </View>
      ))}
    </>
...
const ReferenceItem = ({ reference, styles }: { reference: ReferenceItem; styles: any }) => (
  <View style={{ marginBottom: 10 }} wrap={false}>
    <Text style={styles.itemHeader}>{reference.name}</Text>
    <Text style={styles.itemSubHeader}>{reference.company}, {reference.position}</Text>
    {reference.email && <Text style={styles.description}>Email: {reference.email}</Text>}
    {reference.phone && <Text style={styles.description}>Phone: {reference.phone}</Text>}
  </View>
);

const ReferencesSection = ({ references, styles }: { references: ReferenceItem[]; styles: any }) => {
  const chunks: ReferenceItem[][] = [];
  let currentChunk: ReferenceItem[] = [];
  let currentChunkLength = 0;
  
  const estimateItemSize = () => 0.5;
  ...
  return (
    <>
      {chunks.map((chunk, chunkIndex) => (
        <View key={`ref-chunk-${chunkIndex}`} style={styles.section} break={chunkIndex > 0}>
          {chunkIndex === 0 && <Text style={styles.sectionHeader}>References</Text>}
          {chunk.map((reference, index) => (
            <ReferenceItem key={index} reference={reference} styles={styles} />
          ))}
        </View>
      ))}
    </>
...
const ResumePDFDocument: React.FC<ResumePDFDocumentProps> = ({ resumeData }) => {
  ...
  const styles = React.useMemo(() => createStyles(style), [
    style.fontSize,
    style.lineHeight,
  ...
  return (
    ...
    >
      <Page size="A4" style={styles.page} wrap>
        ...
        {activeSections.includes('summary') && summary && summary.trim() !== '' && (
          <SummarySection summary={summary} styles={styles} />
        )}

        {activeSections.includes('experience') && experience && experience.length > 0 && (
        ...
        {activeSections.includes('certifications') && certifications && certifications.length > 0 && (
          <CertificationsSection certifications={certifications} styles={styles} />
        )}

        {activeSections.includes('references') && references && references.length > 0 && (
